...
당신은 당신의 코드를 고칠 수 있다. 그러나 만약 라이브러리 혹은 너가 의존하는 툴이 동작하지 않는다면, 너가 할 수 있는 것은 없다. 예를 들면 공식적으로 JDK11(2.20.1)을 지원하는 Mockito mocking 프레임워크는 아직 나중 버전을 지원할 준비가 되어있지 않다.
