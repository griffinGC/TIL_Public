...
객체 지향 상속은 물체들을 클래스로 조직화 시키는 것에 대한 것이기 때문에, 실제 세계를 잘 모델링할 수 없다. 모델이 하는 것을 잘 만드는 것은 실세계에 대해 인간이 생각하는 방식이다.



## 메소드도 실제가 아니다

약 20년전에 소프트웨어 밴더의 기술 담당자가 우리회사의 엔지니어들에게 OOP에 대해 설명하던 때의 에피소드가 기억난다. 그는 객체지향이 실 세계를 모델링하는 방법이라고 주장하려고 했고, 그가 제시한 예시는 커피컵 같은 것이었다. 그는 컵은 "drink()" 라는 메소드를 가질 수 있다고 말했다.
